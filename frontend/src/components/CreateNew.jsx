...
const CreateNew = () => {
  ...
  const handleSubmit = async () => {
    ...
    const invalidContributor = contributors.some(
      c => !c.isCreator && (!c.user_id || !c.name),
    );
    ...
    try {
      ...
      for (const c of contributors) {
        await axios.post(
          `/api/v1/hisabs/${hisab.hisab_id}/participants`,
          {
            user_id: c.user_id,
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-indigo-300 flex items-center gap-2">
                <Users size={20} />
                Members ({contributors.filter(c => c.user_id && c.name).length})
              </h2>
              <button
                  ...
                  <div>
                    ...
                    <div
                      className={`text-xl font-bold ${
                        budgetDifference >= 0 ? 'text-cyan-400' : 'text-red-400'
                      }`}
                    >
